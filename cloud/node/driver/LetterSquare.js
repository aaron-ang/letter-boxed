...
class LetterSquare {
     ...
     */
    isValid(letter, wordNum, charNum) {
        ...
        const currWord = this.words[wordNum], newWord = currWord + letter;
        return (!this.onSameSide(letter, LetterSquare.lastLetter(currWord)) &&
     ...
     * appropriate values will be passed in.
     */
    solveRB(wordNum, charNum, maxWords) {
    ...
    solve() {
        ...
        const longest = this.solvingProcess.reduce((a, b) => (a.length > b.length ? a : b), [] // initialValue
        );
        this.solvingProcess.push(longest);
        return { success: false, data: this.solvingProcess };
    }
    solveRBFull(wordNum, charNum, maxWords) {
        let numSolutions = 0;
        // First base case: puzzle solved
        if (this.allLettersUsed() &&
            LetterSquare.dictionary.hasFullWord(this.words[wordNum]) &&
